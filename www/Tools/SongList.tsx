...
type ColumnRenderTuple<T> = [
  ...
  number, // minWidth
  number?, // maxWidth
  ((item: T) => ReactElement)?, // render func
];
 ...
 *          minWidth: number,
 *          maxWidth?: number,
 *          render?:(s:Song) => ReactElement][]} renderers - data for the columns
...
export function ProcessSongGroupData<T>(
  ...
  renderers: [
    ...
    number,
    number?,
    ((song: T, index?: number) => ReactElement)?,
  ][],
  headerRenderer: (group: IGroup) => ReactElement,
  getSort: () => SortKey,
  performSort: (sort: SortKey) => void,
